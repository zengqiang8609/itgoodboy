...
<?php if(defined('IN_ADMIN') && !defined('HTML')) {echo '</div>';}?>
        ...
        <div class="box">
        		<h5>专题</h5>
<div class="pad-lr-10">
<?php if(defined('IN_ADMIN')  && !defined('HTML')) {echo "<div class=\"admin_piao\" pc_action=\"special\" data=\"op=special&tag_md5=97980feaf6c2d763deeaa33e04bbdad3&action=lists&siteid=%24siteid&listorder=3&page=%24_GET%5Bpage%5D&num=20\"><a href=\"javascript:void(0)\" class=\"admin_piao_edit\">编辑</a>";}$special_tag = pc_base::load_app_class("special_tag", "special");if (method_exists($special_tag, 'lists')) {$pagesize = 20;$page = intval($_GET[page]) ? intval($_GET[page]) : 1;if($page<=0){$page=1;}$offset = ($page - 1) * $pagesize;$special_total = $special_tag->count(array('siteid'=>$siteid,'listorder'=>'3','limit'=>$offset.",".$pagesize,'action'=>'lists',));$pages = pages($special_total, $page, $pagesize, $urlrule);$data = $special_tag->lists(array('siteid'=>$siteid,'listorder'=>'3','limit'=>$offset.",".$pagesize,'action'=>'lists',));}?>
    <ul class="list row-2 f14 lh24">
	<?php $n=1;if(is_array($data)) foreach($data AS $r) { ?>
		<li>·<a href="<?php echo $r['url'];?>" target="_blank"><?php echo str_cut($r['title'], 42);?></a></li>
